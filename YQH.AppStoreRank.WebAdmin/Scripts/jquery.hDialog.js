...
;(function($, window, document, undefined) {	
	...
	methods = {
           	...
           	return this.each(function() {
                ...
                if(typeof(_O) == 'undefined') {
						...
						isOverlay: true,            //是否显示遮罩背景(默认true：显示，false：不显示)                        
	                		...
	                		isCloseOverlay: true      //关闭时是否关闭遮罩层
                ...
                if(_O.autoShow != false) {
                    ...
                    T.on('click', function () { methods.open(_O, T); }); //修改为live绑定试一试
                }
			});
        },
        open: function (o, T) {
            ...
            //修改添加
            if (o.box) {
                var isShow = $(o.box).data('show');
                if (isShow) {
                    return;
                }
                $(o.box).data('show', true);
            }
            //修改添加


        		var w,h,t,l,m,$close, headTpl = closeBtnTpl = overlayTpl = iframeTpl = '', $obj = $(o.box), title = o.title, c = T.attr("class"), modalBg = o.modalBg, closeBg = o.closeBg, closecol = o.closecol;
			...
			//关闭按钮
			if(o.closeHide != false) closeBtnTpl = '<a id="HCloseBtn" title="关闭" style="width:24px;height:24px;line-height:24px;display:inline-block;cursor:pointer;background-color:'+closeBg+';color:'+closecol+';font-size:1.5em;text-align:center;position:absolute;top:8px;right:8px;"><span style="width:24px;height:24px;display:inline-block;">×</span></a>';

			//弹框标题
			if(o.title != '') headTpl = '<div id="HTitle" style="padding:10px 20px; border-bottom:1px solid #ddd;overflow:hidden;text-overflow:ellipsis;white-space:nowrap; text-align:center;">'+o.title+'</div>';
			
			//遮罩背景层
			if(o.isOverlay != false) overlayTpl = "<div id='HOverlay' style='width:"+$D.width()+"px;height:"+$D.height()+"px;background-color:"+modalBg+";position:fixed;top:0;left:0;z-index:999999;'></div>";
	    ...
	    close: function (o,T) {
	        ...
	        if (o.isCloseOverlay)
	            methods.remove('#HOverlay,#HTitle,#' + o.iframeId);
	        else
	            methods.remove('#HTitle,#' + o.iframeId);
            ...
            methods.fire.call(this, o.afterHide); //修改添加
	    },	    
    ...
    //这个关闭不再用了。
	//$.fn.hDialogClose = function (obj) {

	//    var effect = "zoomOut";
	//    $('#HOverlay,#HTitle,#iframeHBox').remove();
	//    $(obj).removeAttr('class').addClass('animated '+effect);
	//    if ($(obj).hasClass(effect)) {
	//        setTimeout(function () { $(obj).removeAttr('style').hide(); }, 300);
	//    }
	...
	*/
	$.extend({  
        ...
        dialog: function (type, title, content, time, callback) {
            var t, tpl = '', footerTpl = '', isLock = false, okText = '确定', cancelText = '取消', width = 420, margin = '0 0 0 ' + parseInt(-(width / 2)) + 'px', type = type != undefined ? type : 'alert', time = time != undefined ? parseInt(time) : 0,
            headerTpl = '<div id="hDialog-header"><div class="icon warning pulseWarning" > <span class="body pulseWarningIns"></span> <span class="dot pulseWarningIns"></span> </div></div>',
            okTpl = '<a href="javascript:;" style=" width:100px;padding:5px 0;display:inline-block;border-radius:3px;background-color:#337ab7;color:#fff;" id="h_ok">' + okText + '</a>',
            cancelTpl = '<a href="javascript:;" style="width:100px;padding:5px 0;display:inline-block;border-radius:3px;margin-left:15px;background-color:#eee;color:#333;" id="h_cancel">' + cancelText + '</a>',
            contentTpl = '<div id="hDialog-content" style="padding:0 15px;text-align:center; font-size:20px;">' + content + '</div>',
            ...
            if (type === 'confirm') { footerTpl = '<div id="hDialog-footer" style="padding:20px 10px;text-align:center;">' + okTpl + cancelTpl + '</div>'; }
            if (isLock != true) tpl += maskTpl;
            tpl += '<div id="hDialog-wrap" class="animated fadeIn" style="width:' + width + 'px;border-radius:5px;box-shadow:4px 4px 10px #666;-webkit-box-shadow:4px 4px 10px #666;background-color:#ffffff;position:fixed;top:35%;left:50%;margin:' + margin + ';z-index:100000;">' + headerTpl + contentTpl + footerTpl + '</div>';
